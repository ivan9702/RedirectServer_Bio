...
const minutiae = require('./middleware/minutiae');
const clientUserId = require('./middleware/clientUserId');
...
redirect.post('/delete', clientUserId.check, async (req, res) => {
...
redirect.use(minutiae.check); // minutiae is required for the following routes.
...
redirect.use(clientUserId.check); // clientUserId is required for the following routes.

redirect.post('/enroll', async (req, res) => {
  if (!req.body.fpIndex || !('privilege' in req.body)) {
    return res.json({ code: 40603, message: 'Required Columns Not Fulfilled.' });
  }
